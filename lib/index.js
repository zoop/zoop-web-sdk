...
(function () {
    var options = {
        ...
        studentSDKV1URL: "https://gateway.zoop.one/student-verification-sdk/request_id",
        ocrSDKV1URL: "https://gateway.zoop.one/ocr",
    ...
    zoop.eSignGatewayInit = function (gatewayOption) {
        ...
        zoop.esignGatewayOptions.zoomLevel = zoop.check(
          gatewayOption,
          "zoomLevel"
        )
          ? // only numbers between 0.8 to 2.0 should be sent in query params
            Number.parseFloat(
              0.8 +
                (Math.max(
                  Math.min(Number.parseInt(gatewayOption.zoomLevel), MAX_ZOOM_LEVEL),
                  1
                ) -
                  1) *
                  0.2
        ...
        } else if (gatewayOption.mode && gatewayOption.mode.toUpperCase() === gatewayModes.REDIRECT) {
    ...
    zoop.initDigilockerGateway = function initDigilockerGateway(gatewayOption = {}) {
        if (gatewayOption.mode && gatewayOption.mode.toUpperCase() === gatewayModes.REDIRECT) {
    ...
    zoop.openDigilockerGateway = function openDigilockerGateway(request_id) {
        ...
        if (zoop.digilockerGatewayOption.mode === gatewayModes.TAB) {
          if (
            zoop.options.zoopWindow == null ||
            zoop.options.zoopWindow.closed
          ) {
    ...
    zoop.initLivenessGateway = function initLivenessGateway(
      gatewayOption = {}
    ) {
    ...
    zoop.initStudentSDKGateway = function initStudentSDKGateway(
      ...
      zoop.initOcrSDKGateway = function initOcrSDKGateway(
        gatewayOption = {}
      ) {
